...
const [products,setProducts]=useState();
  ...
  return (
    ...
    container>
    {products&&Array.from(products).map((singleProduct,index) =><Grid align="center" className={styles.grid} item xs={12}  md={6} lg={4}><Product visibility="hidden" moveToProductPage={moveToProductPage}  key={index} price={singleProduct.price} Img={singleProduct.image.asset.url}  title={singleProduct.name}/></Grid>)}
    </Grid>
    <Newsletter/>
    <Footer/>
    </div>
