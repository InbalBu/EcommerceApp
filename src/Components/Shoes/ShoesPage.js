...
function moveToProductPage(singleProduct)
{
  ...
  return (
    ...
    <Cart/>
    <NavbarComponent/>
    <h1 className={styles.header}>Shoes</h1>
    <Grid 
     alignItems="center"
    justify="center" 
    textAlign="center"
    ...
    container>
    {products&&Array.from(products).map((singleProduct,index) =><Grid align="center" className={styles.grid} item xs={12}  md={6} lg={3}><Product moveToProductPage={moveToProductPage}  key={index} price={singleProduct.price} Img={singleProduct.image.asset.url}  title={singleProduct.name}/></Grid>)}
    </Grid>
    <Newsletter/>
    <Footer/>
    </div>
